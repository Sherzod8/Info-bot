...
async def main(call: types.CallbackQuery,state: FSMContext):
    ...
    if call.data=='courses':
        ...
        if user_l.language == 'uz':
            ...
            await call.message.answer_photo(file_path,"🚀 Хотите стать высококвалифицированным IT-специалистом, делать прибыль, повышать свою квалификацию и изучать IT-индустрию на английском языке? 🤔\n\n😎 Наши высококвалифицированные наставники дадут вам необходимые знания во всех областях информационных технологий и помогут вам найти свое место в IT-индустрии. \n\n💥 Мы предоставляем вам знания в области IT на основе современных технологии.\n\n⚡️ Выберите курс по интересующему вас направлению и зарегистрируйтесь.👇👇👇",reply_markup=coursesru)
    ...
    if ss.get('course'):
        ...
        if user_l.language == 'uz':
            ...
            if data[1]=='1':
                ...
                await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 Вы научитесь собирать и анализировать данные, извлекать ценные данные из большого массива информации: отзывов, прогнозов, результатов исследований. Он помогает бизнесу принимать взвешенные решения: строить гипотезы, запускать продукты, улучшать процессы, планировать развитие.\n\n📆 Продолжительность курса: 6 месяцев.\n\n💰 Стоимость курса: 600 000 сум/месяц.").format(kurs_ru[int(id)-1]),reply_markup=joinru)
            elif data[1]=='8':
