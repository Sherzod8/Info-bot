...
async def course(call: CallbackQuery, state:FSMContext):
    ...
    if ss.get('course'): #  and ss.get('center') qoʻshildi
        if data[1]=='0':
            if user_l.language == 'uz':
                await call.message.answer('Iltimos, menyu orqali keyingi qadamni tanlang!',reply_markup=main_manu)
            else:
        ...
        if user_l.language == 'uz':
            ...
            if id=='1':
                ...
                await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 SMM(Social Media Marketing) kursi davomida sizga ijtimoiy tarmoqlarda brendni tanitish, kontent yaratish, va targeting kabi ko‘plab sohaga oid bilim va ko‘nikmalarga ega bo‘lish. Bundan tashqari, sizda real loyihalar bilan ishlab malaka oshirish imkoniyati mavjud bo‘ladi.\n\n📆 Kurs davomiyligi: 3 oy.\n\n💰 Kursning narxi: 1 000 000 so‘m/oy.").format(kurs_uz[int(id)-1]),reply_markup=join)
            elif id=='7':
            ...
            if data[1]=='1':
                ...
                await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 Backend-разработчик - это специалист, который занимается программно-административной частью веб-приложений, внутренним содержанием системы, серверными технологиями, базой данных, архитектурой и логикой программных продуктов.\n\n⚡️ На курсе Вы изучите язык программирования Python, один из самых популярных высокоуровневый языков программирования, а также освоите самые важные Backend-инструменты: Django, базы данных SQL, Git и другое.\n\n📆 Продолжительность курса: 6 месяцев.\n\n💰 Стоимость курса: 1 000 000 сум/месяц.").format(kurs_ru[int(id)-1]),reply_markup=joinru)
            elif data[1]=='3':
                await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 Цель курса – разработать визуальную (пользовательскую) сторону веб-сайтов и предоставить полный спектр знаний и навыков, необходимых для того, чтобы стать младшим (Junior) программистом.\n\n📆 Продолжительность курса: 6 месяцев.\n\n💰 Стоимость курса: 800 000 сум/месяц.").format(kurs_ru[int(id)-1]),reply_markup=joinru)
            elif data[1]=='4':
        ...
        if data[1]=='0':
            ...
            if not ss.get('center'):
                ...
                if data[1]=='1':
                    ...
                    await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 Scratch bolalarni dasturlashga olib kiradigan dasturdir. Oʻquvchilar ushbu dastur yordamida oʻzlarini vizual dasturlash muhitida sinab koʻrishadi.\n\n🖇 Qolaversa «Scratch + IT English» kursi ikki qismga boʻlingan boʻlib, o’quvchilar darsning birinchi qismida Ingliz tilini, ikkinchi qismida Scratchni oʻrganishadilar. Dastur toʻliq amaliyotga asoslangan: har bir darsdan soʻng amaliy mashqlar, qiziqarli fikr-mulohazalarga boy boʻladi.\n\n📆 Kurs davomiyligi: 4 oy.\n\n💰 Kursning narxi: 1 000 000 so‘m/oy.").format(kurs_uz[int(id)-1]),reply_markup=join)
                elif data[1]=='8':
                    await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 IT-English kursi xalqaro miqyosida tan olingan standartlarga muvofiq, innovatsion usulda bo‘lib o‘tadi. Siz CEFR va IELTS imtihonlaridan muvaffaqiyatli o‘tishingiz uchun innovatsion tayyorgarlikka ega bo‘lasiz.\n\n📆 Kurs davomiyligi: 6 oy.\n\n💰 Kursning narxi: 460 000 so‘m/oy.").format(kurs_uz[int(id)-1]),reply_markup=join)
                await call.answer(cache_time=0.02)
                await Anketa.choice.set()
        ...
        if data[1]=='0':
            ...
            if not ss.get('center'):
                ...
                if data[1]=='1':
                    ...
                    await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 Цель курса – разработать визуальную (пользовательскую) сторону веб-приложений и предоставить полный спектр знаний и навыков, необходимых для того, чтобы стать junior программистом.\n\n📆 Продолжительность курса: 6 месяцев.\n\n💰 Стоимость курса: 800 000 сум/месяц.").format(kurs_ru[int(id)-1]),reply_markup=joinru)
                elif data[1]=='4':
                    await call.message.answer_photo(file_path,caption=("<b>📌 {}</b>\n\n👨‍💻 Целью курса является создание сложных изображений и контента с помощью графических программ, повышение творческих способностей учащихся, профессиональное освоение таких программ, как Adobe Photoshop, Adobe Illustrator и Corel Draw.\n\n📆 Продолжительность курса: 4 месяца.\n\n💰 Стоимость курса: 800 000 сум/месяц.").format(kurs_ru[int(id)-1]),reply_markup=joinru)
                elif data[1]=='5':
